...
Three findings, all open. F-001 (MEDIUM, scope drift on save side from BOM stripping) and F-002 (MEDIUM, alert host not pinned for the no-document case) are the load-bearing ones — both name reachable user-observable failure modes that the design as written does not close. F-003 (LOW) is a one-sentence ordering clarification across the feature boundary with `resume-and-detector-hardening-11`. No HIGH findings; no integrity contradictions; no standards-compliance issues against `constitution.md`.
