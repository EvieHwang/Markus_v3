...
| T-006 | `LaunchResumeBranch` (C2) — resume-vs-browser decision at scene activation | 4 | in-progress | |
| T-007 | `CreateDocumentHandler` (C4) — deferred-write new-file flow | 4 | in-progress | |
| T-008 | `DocumentOpenObserver` (C3) + `BackToBrowser` (C8) — record-on-open and leading back affordance | 4 | in-progress | |
