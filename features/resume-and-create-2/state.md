...
|----|-------------|------|--------|-------|
| T-001 | `LastFileStore` (C1) — durable last-opened reference | 1 | in-progress | |
| T-002 | `NameProbe` (C5) — collision-free `Untitled` naming | 1 | in-progress | |
| T-003 | `LocalDocumentsFallback` (C7) — always-available create target | 1 | in-progress | |
| T-004 | `CreateTargetResolver` (C6) + `LastDirectoryProviding` — target directory + writability probe | 2 | pending | |
