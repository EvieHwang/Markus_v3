...
| T-006 | `LaunchResumeBranch` (C2) — resume-vs-browser decision at scene activation | 4 | complete | 28fcae3 — resume fires from host viewDidAppear; UI-test launch args wired |
| T-007 | `CreateDocumentHandler` (C4) — deferred-write new-file flow | 4 | complete | 28fcae3 — D-007 additive params on DocumentView |
| T-008 | `DocumentOpenObserver` (C3) + `BackToBrowser` (C8) — record-on-open and leading back affordance | 4 | complete | 28fcae3 — edge-swipe via D-006; back chevron via D-007 |
