...
- AC-4.4: The new file is created on disk **with zero bytes** as a placeholder for the `UIDocumentBrowserViewController` handoff. It is opened immediately into the document view in **raw mode** with the **keyboard active** and the cursor at the start of the document. The raw-mode + keyboard-up behavior is triggered by **session provenance, not byte size** — specifically, a file that was created via `CreateNewDocumentFlow` during the current app session. Files opened from any other path (browser pick, resume, share import) continue to follow walking-skeleton-1 EC-2's mode-from-byte-size default: files < 500 KB open in rendered mode, files ≥ 500 KB open in raw mode. A zero-byte file on disk that is opened via the resume path (e.g., the user created `Untitled.md`, the app died before any keystroke, the file was therefore not cleaned up, and the next launch resumes it) opens in rendered mode per walking-skeleton-1 EC-2 — it is no longer in the create-flow session set. Implementation note: the session-set membership is held by `UntouchedFileTracker`; `DocumentView`'s initial-mode decision consults `UntouchedFileTracker.shared.isUntouched(url:)` first and falls back to byte-size logic only when the URL is not in the set.
...
**Third pass — folds in design RI-4 and adversarial F-001.** Two changes:

- **AC-3.3 rewritten (again)** — reverts to the system-provided `UINavigationController.interactivePopGestureRecognizer`. The prior pass had rewritten this to use a custom `UIScreenEdgePanGestureRecognizer` because the then-design used a present-with-transition pattern with no nav stack. The current design (third-pass) adopts the placeholder-VC pattern (design.md component #2 / adversarial F-005), which gives `documentVC` a real parent in the nav stack — so `interactivePopGestureRecognizer` is natively active again. This restores the natural Apple gesture intended by the declaration. (Addresses design RI-4.)
- **AC-4.4 rewritten** — the raw-mode + keyboard-up trigger is now **session provenance** (created via `CreateNewDocumentFlow` this session), not byte size. Previously the AC said "regardless of byte size" but the design implemented it as a byte-size check, which produced a contradiction with EC-23 ("zero-byte file on disk opened via resume opens to an empty rendered view"). The new wording explicitly delegates the membership check to `UntouchedFileTracker.shared.isUntouched(url:)`. EC-23's behavior is now consistent. (Addresses adversarial F-001.)

**Second-pass content retained:** all six user stories, all 23 edge cases (EC-23 unchanged — it now aligns with the corrected AC-4.4), the standards check, and the prior architectural decisions (RI-1, RI-2, RI-3).

**First-pass content retained:** original user stories and edge cases.

Requirements stable — no further architecture changes pending. Re-running `/t3-architecture` should report stable on its side (the design already references `UntouchedFileTracker.isUntouched` in component #8 in anticipation of this change), closing the loop. Re-running `/t3-adversarial` should verify F-001 as resolved alongside F-002 through F-006.
