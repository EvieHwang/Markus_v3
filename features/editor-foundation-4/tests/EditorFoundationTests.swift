...
struct UITextViewMigrationTests {
    ...
    func markDirtyDoesNotMutateText() {
        let source = "Some content"
        let doc = MarkdownDocument(text: source)
        // markDirty requires an UndoManager; test that the text is unchanged
        // after the document is used (constructor path only in spec tests)
        #expect(doc.text == source)
    }
}
