# State: editor-foundation-4

| ID | Description | Wave | Status | Notes |
|----|-------------|------|--------|-------|
| T-001 | `ScrollAnchor` value type | 1 | complete | a8b6b7d |
| T-002 | `RawEditorScrollState` observable object | 1 | complete | a8b6b7d |
| T-003 | `ListContinuationHandler` pure value type | 1 | complete | a8b6b7d |
| T-004 | `MarkdownEditorTextView` UITextView subclass | 1 | complete | a8b6b7d |
| T-005 | `MarkdownTextViewBridge` UIViewRepresentable | 2 | complete | d90bcc3 |
| T-006 | Replace `RawEditorView` body with `MarkdownTextViewBridge` | 3 | complete | 67887aa |
| T-007 | `DocumentView` anchor wiring + `RenderedView` tap/anchor extension | 4 | complete | 3a296a2 |
