...
- BR-20.4 Suspension lifts on resolution of the latched outcome — either an explicit user resolution (Keep Mine performs the single deliberate write (BR-5); Keep Theirs / Discard Mine / Save As perform no write that clobbers the content the collision was about) or the recoverable lift defined in BR-21.3 when a non-user dismissal occurred and the underlying condition no longer holds. Normal autosave resumes only after the collision (or deletion) is resolved by one of these paths — never silently while it is still unresolved.

### BR-21 — An unresolved collision or deletion is never silently lost to a non-user dismissal
**As a** user who has a conflict sheet or deletion banner pending,
**when** the system (not I) dismisses that surface — for example because the app is backgrounded or the OS tears the surface down under memory pressure — without my having made an explicit choice,
**so that** a collision or deletion I never resolved can never be silently forgotten and my edits never silently overwritten,
**I want** the unresolved state preserved and re-presented when the app/document returns to the foreground, with save-back remaining safely suspended until I explicitly resolve.
*Addresses adversarial F-003 (requirements portion); strengthens BR-15 and BR-20.4.*

Acceptance criteria:
- BR-21.1 A non-user dismissal — the conflict sheet or deletion banner disappearing for any reason other than an explicit user resolution (Keep Mine / Keep Theirs / Discard Mine / Save As, or the explicit user dismissal of the deletion banner per BR-9.6) — does NOT mark the collision or deletion as resolved. The unresolved outcome is retained.
- BR-21.2 While an unresolved collision or deletion persists after a non-user dismissal, save-back and autosave remain suspended (per BR-20): no write to the followed (or vanished) location occurs, and the disagreeing on-disk content the collision was about — or the absence the deletion was about — stays intact and recoverable. Save-back does not silently resume merely because the surface disappeared.
- BR-21.3 When the app or document returns to the foreground with an unresolved collision or deletion still latched and its surface no longer presented, the unresolved state is re-surfaced: the conflict sheet (or deletion banner) is presented again so the user can complete the choice — OR, if the underlying condition no longer holds when re-evaluated against the current settled disk content and live buffer (e.g. the buffer is now content-identical to disk, or the file has reappeared as a move), the outcome is cleared, suspension lifts, and normal autosave resumes. In neither case does suspension persist with no surface and no path to resolution.
- BR-21.4 Suspension can never outlive the surface indefinitely: there is no observable end-state in which an unresolved collision or deletion is latched, no surface is presented, and save-back/detection remain suspended with no re-present and no recovery — i.e. the "stuck-suspended, silently-forgotten" state must not be reachable.
- BR-21.5 The buffer is preserved in memory across the non-user dismissal and the re-presentation (consistent with BR-15 and BR-9.6); no forced data loss occurs as a result of the system dismissing the surface.
...
### Recoverable lift after non-user dismissal added in the F-003 revision pass

7. **Recoverable lift / re-present of a latched-but-unresolved outcome after a non-user dismissal (BR-21).** The F-002 fix (DC-22) keyed save-back suspension to the latched classified outcome and stated it "lifts only on explicit resolution," which removed the implicit resume-on-dismissal of the prior "while the surface is up" scoping. Requirements now fix the *behavior* — a non-user dismissal (backgrounding, memory-pressure teardown) must never mark a collision/deletion resolved; the unresolved state and its suspension must be preserved and, on return to the foreground, re-presented (or recoverably lifted when the underlying condition no longer holds), so suspension can never outlive the surface indefinitely (BR-21). The *mechanism* is an architecture concern: design must define a recoverable lift/re-present condition (not "only on explicit resolution"), specifying where the latched outcome is detected as surface-less on foreground, how it re-validates against current settled disk + live buffer (reusing DC-21), and the behavioral lift condition that replaces DC-22's "only on explicit resolution." This requires tightening DC-22 (bullet 4) and DC-15, and anchoring the new mechanism to BR-15 / BR-20.4 / BR-21. *Addresses adversarial F-003 (architecture portion).*

The following still needs architectural resolution before requirements can be marked fully stable:

- **F-003 mechanism (item 7 above).** BR-21 fixes the behavior — no unresolved collision/deletion is silently lost to a non-user dismissal; the state is preserved, re-presented or recoverably lifted on foreground, and never stuck-suspended. The design mechanism for this (the foreground re-validate-and-re-present/lift path, and the corresponding tightening of DC-22 bullet 4 and DC-15) has not yet been supplied. Until design adds it, BR-21's "never stuck-suspended" and "re-present on return" criteria have no mechanism anchor for the spec tests to bind to. This is why the stability marker below is OFF.
