...
Item 7 above is now resolved by design.md: the recoverable lift / re-present of a latched-but-unresolved outcome after a non-user dismissal (DC-23, with DC-22 bullet 4 and DC-15 tightened, addressing F-003 architecture portion, anchoring BR-15 / BR-20.4 / BR-21). The foreground reconciliation re-validates the latched outcome against current settled disk + live buffer (reusing DC-21) and either re-presents the surface or recoverably lifts suspension; the stuck-suspended/surface-less state is made unreachable. No requirement *text* changed — BR-21 already fixed the behavior; design supplied only the mechanism.
