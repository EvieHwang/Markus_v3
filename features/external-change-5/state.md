...
| T-004 | `ChangeClassifier` (DC-4) — four exclusive outcomes, presence-first | 2 | in-progress | |
| T-005 | `SettleGate` (DC-6, DC-7, DC-8) — 2s window + in-flight suppression, delay-not-discard | 2 | in-progress | |
| T-006 | `ApplyEdgeRevalidation` (DC-21) + `SaveSuspensionLatch` (DC-22) — apply-edge integrity | 2 | in-progress | |
