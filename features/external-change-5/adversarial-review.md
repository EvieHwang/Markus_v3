...
- **Severity:** HIGH
- **Status:** addressed — requirements portion by BR-19 (BR-19.1–BR-19.4); architecture portion now complete via design.md DC-21 (§Apply-edge integrity): an outcome is re-validated against the live buffer before any mutation, so an absorb derived from an off-main read can never overwrite edits typed during the read.
...
- **Severity:** HIGH
- **Status:** addressed — requirements portion by BR-20 (BR-20.1–BR-20.4); architecture portion now complete via design.md DC-22 (§Apply-edge integrity, tightening DC-14/DC-16): autosave/save-back is suspended from the instant `collision` or `deleted` is classified — not from when the surface is presented — closing the classify→present latency gap.
- **Lens:** failure modes / integrity
...
Open findings: 0 HIGH, 0 MEDIUM, 0 LOW. (F-001, F-002 fully addressed; prescription feedback resolved.)
