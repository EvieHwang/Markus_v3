...
**F-003 revision pass (this step).** This pass supplies the remaining open architecture mechanism — the recoverable lift / re-present of a latched-but-unresolved outcome after a non-user dismissal (BR-21, adversarial F-003 architecture portion): §Apply-edge integrity DC-23 (the durable latched outcome + foreground reconciliation that either re-presents or recoverably lifts), with DC-22 bullet 4 and DC-15 tightened so a system dismissal can no longer count as a resolution or strand suspension. This resolved only the *mechanism*; BR-21 already fixed the behavior, so no requirement *text* changed. Accordingly the requirements bottom marker is "Requirements stable — no architectural feedback to incorporate."
...
**DC-15 — Sheet survives backgrounding without auto-resolving; a non-user dismissal never counts as a resolution. (Tightened for adversarial F-003.)** If the app is backgrounded while the sheet (or banner) is up, the pending choice is not auto-resolved in a way that loses the buffer; on return the user can still complete the choice, and if the system dismissed the surface the buffer is preserved in memory (BR-15). The property: backgrounding never silently picks a winner. Tightened: the *only* events that mark a latched `collision`/`deleted` outcome resolved are explicit user resolutions — Keep Mine / Keep Theirs / Discard Mine / Save As, or the explicit user dismissal of the deletion banner (BR-9.6). Any other way the surface disappears — backgrounding, OS memory-pressure teardown, any system-initiated dismissal — leaves the outcome **latched and unresolved**; it does not clear the outcome, does not lift suspension, and does not re-arm the detector on its own (BR-21.1). Recovery from that surface-less latched state is owned by DC-23.
...
*These constraints close the HIGH adversarial findings on the detector's apply edge. Each is stated as an observable end-state property — what the user can never lose, and what the disk can never gain — independent of which actor or queue the work runs on. They constrain the **moment an outcome is applied** (and, for DC-23, the moment a latched outcome is reconciled on return), not how detection samples state. Addresses adversarial F-001, F-002, F-003.*
...
- Suspension lifts on resolution of the latched outcome by one of two paths, and never silently while it is still unresolved (BR-20.4): **(a) explicit user resolution** — Keep Mine performs the single deliberate write (DC-13, BR-5); Keep Theirs / Discard Mine / Save As perform no write that clobbers the content the collision was about; or **(b) the recoverable lift of DC-23** — when the surface was dismissed by the system (not the user) and, on foreground/document-return, the latched outcome no longer holds against current settled disk + live buffer. Outside these two paths, normal autosave does not resume. *(Bullet tightened for adversarial F-003: the prior "lifts only on explicit resolution" wording is replaced by this two-path lift so a non-user dismissal can no longer strand suspension; see DC-23.)*
...
**DC-23 — A latched-but-surface-less outcome is recoverably re-presented or lifted on foreground/return; suspension can never outlive the surface. (Addresses adversarial F-003; replaces DC-22 bullet 4's "only on explicit resolution"; reuses DC-21; anchors BR-15 / BR-20.4 / BR-21.)** The latched classified outcome (`collision` / `deleted`) is durable, not bound to whether its surface is currently on screen. It persists across backgrounding and any system-initiated dismissal, together with its save-back suspension and the detector's quiescence (DC-5). The behavioral properties:

- **No silent forgetting on system dismissal.** A non-user dismissal of the conflict sheet or deletion banner never marks the outcome resolved and never lifts suspension; the disagreeing on-disk content the collision was about — or the absence the deletion was about — stays intact and recoverable, and the buffer is preserved in memory across the dismissal (BR-21.1, BR-21.2, BR-21.5). Save-back does not resume merely because the surface vanished.
- **Foreground reconciliation is the recovery point.** When the app or document returns to the foreground with an outcome still latched and *no* surface presented, the latched-and-surface-less condition is detected and the outcome is re-validated against the *current settled disk state and the live buffer*, reusing the apply-edge re-derivation of DC-21 (and the equality gate DC-11, the presence ordering DC-4/DC-16). Exactly one of two things then happens, with no third "do nothing" branch:
  - **Re-present** — if the outcome still holds (the collision still materially differs against the live buffer, or the file is still genuinely absent), the conflict sheet or deletion banner is presented again so the user can complete the choice. Suspension and quiescence continue across the re-present, unchanged (BR-21.3, first branch).
  - **Recoverable lift** — if the outcome no longer holds (the buffer is now content-identical to settled disk, or the file has reappeared as a `moved` successor), the latched outcome is cleared, save-back suspension lifts, the detector re-arms, and normal autosave resumes — exactly as if the user had been shown nothing because there is nothing to decide (BR-21.3, second branch). This lift is itself a non-clobbering transition: it adopts/retargets per the now-valid outcome (DC-12 absorb-equivalent, or DC-19 move-retarget) rather than writing the stale buffer over disk.
- **No stuck-suspended end-state.** There is no reachable observable state in which an outcome is latched, no surface is presented, and save-back/detection remain suspended with neither a re-present nor a lift available: the foreground reconciliation above is total over the latched-and-surface-less case, so suspension can never outlive the surface indefinitely (BR-21.4). Equivalently, the set {latched, surface-less, suspended, not-being-reconciled} is empty.

The observable test surface: classify a collision (sheet up), background the app, have the system dismiss the sheet, then return to foreground — assert either the sheet is presented again (outcome still holds) or autosave/detector are demonstrably live again (outcome no longer holds, e.g. disk now agrees), and in neither case is the buffer lost or the disagreeing disk content silently overwritten; the "frozen, no surface, suspended forever" state is never observed.
...
- Recoverable lift after non-user dismissal (DC-23): background with a sheet/banner pending, let the system dismiss it, return to foreground → either the surface is re-presented (outcome still holds) or autosave/detector are demonstrably live again (outcome no longer holds — disk now agrees, or file reappeared as a move); the buffer is preserved across the dismissal, the disagreeing disk content is never silently overwritten, and the "latched + surface-less + suspended forever" state is never reachable.
