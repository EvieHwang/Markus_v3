...
8. **Recents integration with the system browser** — files opened via non-pick paths (last-file resume, deferred-write create on first persist, follow-on-move retarget) are registered with `UIDocumentBrowserViewController` so they appear in the system Recents tab alongside files opened via the browser pick. The system browser stays the authority; Markus does not maintain its own recents list (no library, no index). Touches: File access layer, Document browser entry.
