...
/// The four exclusive classification outcomes the detector emits (DC-4). The spec
/// test declared a local mirror; in the Xcode target it binds directly to the real
/// `ExternalChangeOutcome` vended by `ChangeClassifier` / the detector — the case
/// set is the contract (BR-10 / DC-4). (Build deviation D-004: seam mirror → real
/// type alias, per the spec-test header's "build agent maps seam names onto the
/// real symbols." The `.suppressed` mirror case was unused by any assertion.)
typealias ExternalChangeOutcomeKind = ExternalChangeOutcome
